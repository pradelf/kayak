...
def liste_villes() -> list:
    ...
    for lieu in lieux:
        url = (
            f"https://www.booking.com/searchresults.fr.html?ss={lieu.replace(' ', '+')}"
        )
        mapping[url] = lieu
...
class BookingSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for hotel in hotels:
            if (
                hotel.xpath(
                    "div[1]/div[2]/div/div/div[1]/div/div[1]/div/h3/a/div[1]/text()"
                ).get()
                != None
            ):
                # logging.info(room.xpath('div[1]/div[2]/div/div/div[1]/div/div[1]/div[1]/div//div/@aria-label').get())#/@aria-label
                stars = hotel.xpath(
                    "div[1]/div[2]/div/div/div[1]/div/div[1]/div[1]/div//div/@aria-label"
                ).get()
                url = hotel.xpath(
                    "div[1]/div[2]/div/div/div[1]/div/div[1]/div/h3/a/@href"
                ).get()
                nom = hotel.xpath(
                    "div[1]/div[2]/div/div/div[1]/div/div[1]/div/h3/a/div[1]/text()"
                ).get()
                description = (
                    hotel.xpath("div[1]/div[2]/div/div/div[1]/div/div[3]/text()").get(),
                )
                hotel = {
                    "ville": ville,
                    "nom": nom,
                ...
                yield response.follow(
                    url=url, callback=self.parse_coordinates, meta={"hotel": hotel}
                )
        try:
